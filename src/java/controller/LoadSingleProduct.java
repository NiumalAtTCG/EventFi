    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (Validations.isInteger(productID)) {
                ...
                Criteria criteria1 = session.createCriteria(Event.class);
                criteria1.add(Restrictions.eq("category", product.getEvent().getCategory()));
                List<Event> eventList = criteria1.list();

                Criteria criteria2 = session.createCriteria(Product.class);
                criteria2.add(Restrictions.in("event", eventList));
                criteria2.add(Restrictions.ne("id", product.getId()));
                //product 4 kata limit krnna puluwm
